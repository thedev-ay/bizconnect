// WorkflowStage comes from the DB — no hardcoded status list
export interface WorkflowStage {
  id: string;
  name: string;
  slug: string;
  color: string; // zinc | blue | orange | violet | emerald | red | amber | sky
  sortOrder: number;
  type: "active" | "completed" | "cancelled";
}

// Color map — full Tailwind class strings (must be static for purge)
...
export const DEFAULT_COLOR = STAGE_COLOR_MAP.zinc;

export function getStageColors(color: string) {
  return STAGE_COLOR_MAP[color] ?? DEFAULT_COLOR;
}

// Derive next active stage from an ordered list
...
// Derive the "completed" stage (payment trigger)
