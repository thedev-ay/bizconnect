...
type DataSurfaceLoadingVariant = "table" | "kanban" | "cards" | "panel";
        ...
        >
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0 flex-1 space-y-2">
              ...
              <div className="h-4 w-40 rounded-full bg-muted/80" />
              <div className="h-3 w-20 rounded-full bg-muted/45" />
            </div>
            <div className="h-8 w-8 rounded-full bg-primary/10" />
          </div>
          <div className="mt-4 grid grid-cols-3 gap-2">
            <div className="h-10 rounded-2xl bg-muted/35" />
            <div className="h-10 rounded-2xl bg-muted/35" />
            <div className="h-10 rounded-2xl bg-muted/35" />
          </div>
        </div>
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-3">
      {Array.from({ length: 3 }).map((_, laneIndex) => (
        <div
          key={laneIndex}
          className="overflow-hidden rounded-[28px] border border-border/60 bg-background/88"
        >
          <div className="flex items-center justify-between border-b border-border/50 bg-muted/25 px-4 py-3">
            ...
            <div className="h-6 w-8 rounded-full bg-primary/10" />
          </div>
          <div className="space-y-3 p-3">
            {Array.from({ length: 3 }).map((_, cardIndex) => (
              <div
                key={cardIndex}
                className="rounded-[22px] border border-border/55 bg-white/96 p-3 shadow-[0_14px_28px_-24px_rgba(15,23,42,0.18)]"
              >
                <div className="flex items-center justify-between gap-2">
                  <div className="h-3 w-24 rounded-full bg-muted/55" />
                  <div className="h-2.5 w-2.5 rounded-full bg-primary/35" />
                </div>
                <div className="mt-3 h-4 w-28 rounded-full bg-muted/75" />
                <div className="mt-2 h-3 w-20 rounded-full bg-muted/45" />
                <div className="mt-4 h-9 rounded-full bg-muted/35" />
              </div>
            ))}
          </div>
        </div>
